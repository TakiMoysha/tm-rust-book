...
use commands::{Command, Edit, System};
use messagebar::MessageBar;
use position::Position;
use size::Size;
use statusbar::StatusBar;
use terminal::Terminal;
use ui_components::UIComponent;
use view::View;
...
impl Editor {
    pub fn new() -> Result<Self, Error> {
        ...
        if let Some(file_name) = args.get(1) {
            if editor.view.load(file_name).is_err() {
                editor
                    .message_bar
                    .update_msg(&format!("ERR: Could not open file: {file_name}"));
            }
        }
    ...
    pub fn run(&mut self) {
        loop {
            ...
            match read() {
                ...
                Err(err) => debug_panic!("Could not read from stdin: {err}"),
            };
    ...
    fn process_command(&mut self, command: Command) {
        ...
        match command {
            ...
            Command::System(System::Dismiss) => {
                if self.command_bar.is_none() {
                    self.dismiss_prompt();
                    self.message_bar.update_msg("Save aborted.");
                }
            }
    ...
    fn dismiss_prompt(&mut self) {
        ...
        self.message_bar.set_redraw_flag(true);
    }
    ...
    fn show_prompt(&mut self) {
        ...
        command_bar.set_redraw_flag(true);
    ...
    fn save(&mut self, file_name: Option<&str>) {
        let res = if let Some(name) = file_name {
    ...
    fn handle_quit(&mut self) {
        if !self.view.get_status().is_modified || self.quit_times + 1 == QUIT_TIMES {
            ...
            self.message_bar.update_msg(&format!(
