...
fn handle_connection(mut stream: TcpStream) {
    ...
    let request_header = request_line
        .split_whitespace()
        .take(3)
        .collect::<Vec<&str>>();
    let (method, path, http_version) = (request_header[0], request_header[1], request_header[2]);
    ...
    let (status_line, contents) = match &request_header[..] {
        ["GET", "/robots.txt", _] => (
            "HTTP/1.1 200 OK",
            fs::read_to_string("./public/robots.txt").unwrap(),
        ),
        ["GET", "/", _] => (
            "HTTP/1.1 200 OK",
